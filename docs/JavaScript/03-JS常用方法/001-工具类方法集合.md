...
```js
const decryptWithWebCrypto = async (data) => {
	const keyStr = '0000b81740beba80'; // 16字符 UTF-8 字符串
	const ivStr = '20240920a1b2c3d4'; // 16字符 UTF-8 字符串

	// Step 1: 将 Hex 密文转为 Uint8Array
	...
	const ciphertextBytes = hexToBytes(data); // 密文字节数组

	// Step 2: 将 keyStr 和 ivStr 用 UTF-8 编码为字节
	const encoder = new TextEncoder(); // UTF-8 编码器
	const keyBytes = encoder.encode(keyStr); // 16 字节
	const ivBytes = encoder.encode(ivStr); // 16 字节
