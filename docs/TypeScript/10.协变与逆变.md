...
| 变型 | 规则 | 适用场景 | 记忆口诀 |
|------|------|--------|--------|
| 协变（Covariant） | `A extends B` ⇒ `Container<A> extends Container<B>` | 只读（输出） 场景，如返回值、数组元素 | 子 → 父，安全（内容更具体） |
| 逆变（Contravariant） | `A extends B` ⇒ `Container<B> extends Container<A>` | 只写（输入） 场景，如函数参数 | 父 → 子，安全（要求更宽泛） |
| 不变（Invariant） | 即使 `A extends B`，`Container<A>` 与 `Container<B>` 也无子类型关系 | 既读又写（可变）场景，如可变数组 | 默认行为，最安全 |
